...
export const Chatbot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Get AI response from Gemini with RAG-enhanced context
      // Pass all transactions for RAG retrieval system
      const currentTransactions = visibleTransactions.length > 0 ? visibleTransactions : transactions;
      ...
      let errorContent = 'I apologize, but I\'m having trouble analyzing your data right now. Please try again in a moment.';

      if (error instanceof Error) {
        if (error.message.includes('API key not configured')) {
          errorContent = 'The Gemini API key is not configured. Please add VITE_GEMINI_API_KEY to your .env file and restart the app.';
        } else if (error.message.includes('API_KEY_INVALID') || error.message.includes('invalid')) {
          errorContent = 'The Gemini API key appears to be invalid. Please check your .env file and ensure the key is correct.';
        } else {
          errorContent = `Error: ${error.message}. Please check your API key configuration.`;
        }
      }
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 h-[500px] sm:h-[600px] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-4">
          {messages.map((message) => (
            ...
            >
              <div className={`flex items-start space-x-2 sm:space-x-3 max-w-xs sm:max-w-sm lg:max-w-md ${message.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''
                }`}>
                ...
                <div className={`px-3 sm:px-4 py-2 sm:py-3 rounded-2xl relative ${message.type === 'user'
